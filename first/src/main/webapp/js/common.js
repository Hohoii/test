...
function ComAjax(opt_formId){
    ...
    //ajax를 이용하여 데이터를 전송한 후 호출될 콜백함수의 이름을 지정
    ...
    this.ajax = function ajax(){
        ...
        $.ajax({
            url : this.url,   
            type : "POST",  
            data : this.param, //ajax를 이용하여 서버에 요청을 할 때 서버로 전달할 인자(Parameter)
            async : false, //통신방식= 비동기(동기: '클라이언트 -> 서버 -> 클라이언트'의 과정에서 서버의 답이 올때까지 다른일x / 비동기: 요청보내고 다른일 수행가능)
...
function gfn_renderPaging(params){
	...
	var totalIndexCount = Math.ceil(totalCount / recordCount); // 전체 페이지 인덱스 수
	gfv_eventName = params.eventName;
	
	$("#"+divId).empty();
	var preStr = ""; //맨앞으로 이동
	var postStr = ""; //맨 뒤로 이동
	var str = ""; //1~10 등과 같은 인덱스
